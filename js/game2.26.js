	...
	var game = {
		...
		addEvent: function() {
			...
			$('.viewBtn').on(START_EV, function() {
				window.location.href = '/spirit/onlookers.do'
			})
				...
				_this.startGame(function(data){
					...
					if(data.normal) {
						...
						if(data.msg == "EndTask") {
							...
							return;
						} else if(data.msg == "AppidError") {
							//_this.msgShow('');
							 window.location.reload();
						} else if(data.msg == 'NoActivity') {
							_this.msgShow('您还没有领取活动任务！')
						} else if(data.msg = 'NoOpenTask') {
							_this.msgShow('分享朋友圈可多玩一次，每天最多可参与2次！')
						} else if(data.msg = 'LimitOf') {
							_this.msgShow('今天的计划已经用完，欢迎明天继续冲刺！')
						}else if(data.msg = 'FailTask') {
							_this.msgShow('请稍后再试！')
						}
			...
			$('.btnIco').on(END_EV, function() {
				...
				//规则
					...
					//					$('.rulePanel').show();
					window.location.href = '/spirit/rule.do'
					...
					var bb = document.getElementById("audio");
						if (!bb.paused) {
				...
				$('.successPanel').find('.backBtn').on(END_EV, function() {
					...
					_this.eggGame(function(data) {
						...
						if(data.normal) {
							...
							window.setTimeout(function() {
							if(msg.cardId==4){
								$('.success_text').html('恭喜你砸中了'+msg.cardName);
							}else{
								$('.success_text').html('谢谢参与，再接再厉！');
							}
							}, 2000)
						} else {
							$('.successBox').hide()
							...
							if(data.msg == "NoActivity") {
								...
								_this.msgShow('没有开启活动！');
							} else if(data.msg == 'FailTask') {
								_this.msgShow('请稍后再试！')
							} else if(data.msg = 'NotRepeatTask') {
								_this.msgShow('没有查到您的任务！')
							}else if(data.msg = 'AppidError'){
								 window.location.reload();
							}
						}
...
//				$('.failPanel').find('.success_txt').text(this.currentLevel);
//			}
		...
		setLevelNum: function() {
			var levelNum = localStorage.getItem('levelNum');
			if(levelNum) {
				localStorage.setItem('levelNum', Number(levelNum)+1);
			} else {
				localStorage.setItem('levelNum', 1);
			}
		},
