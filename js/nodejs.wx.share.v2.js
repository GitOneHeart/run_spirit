 ...
 * 
   var _wxData = {
        link:location.href.split('#')[0],//分享的链接
        title:'title',//标题
        imgUrl:'http://weixin.00ok.cc/bingshijie/_static/images/public/wx.png',//分享的图标
        desc:'desc',//描述
        success:function(res){}//分享成功
    }
   _leonWxShare.share(wxData,callback)
 */
...
var _leonWxShare= {
	...
	//用来请求成签名的URL
	createSignUrl:'http://hb.00ok.cc:8299/rsx/1/',//生成签名的NodeJs程序
	/**
	 * 初始化
	...
	init:function(wxData,callback){
		...
		this.xmlHttp.onreadystatechange = function(){
	        if(_this.xmlHttp.readyState == 4){
	        	_this.handleStateChange(function(data){
	                ...
	                //调用微信分享插件
	                _this.share(_this.wxData,function(){
	    ...
	    this.xmlHttp.open("POST", this.createSignUrl, false);
	    //this.xmlHttp.send(null);
	    this.xmlHttp.setRequestHeader("Content-Type","application/x-www-form-urlencoded;charset=UTF-8");
		this.xmlHttp.send("url="+escape(this.wxData.r_link?this.wxData.r_link:this.wxData.link)); 
	},
	...
	share:function(wxData,callback){
	    ...
	    var _title  = wxData.title,
	        _desc   = wxData.desc,
	        _imgUrl = wxData.imgUrl,
	        _link   = wxData.link,
	        _this   = this;
	    
	    function _wxReadyShare(){
	        //分享的数据
	        var _wxData={
	            'title':_title,//分享的标题
	            'link':_link,//分享的链接
	            ...
	            'success':wxData.success?wxData.success:function(res){
	                //if(res.errMsg == 'shareTimeline:ok');//表示已成功分享到朋友圈
	                //if(res.errMsg == 'sendAppMsg:ok');//表示已成功分享给朋友
	                if(callback)callback(res);
	            }
	        },
	        _wxData2={'title':_wxData.desc, 'link':_wxData.link, 'imgUrl':_wxData.imgUrl,'success':_wxData.success}
	        wx.onMenuShareAppMessage(_wxData);//分享给好友
	        wx.onMenuShareQQ(_wxData);//分享到QQ
	        wx.onMenuShareWeibo(_wxData);//分享到微博
	        wx.onMenuShareTimeline(_wxData2);//分享到朋友圈
	    }
	    //weixin ready
	    if(this._isWxReady){
	        _wxReadyShare();
